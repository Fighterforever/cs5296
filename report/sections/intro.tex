...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=2pt]
  \item A reproducible benchmarking harness in which the same image is
        ...
        environment that the course uses (Section~\ref{sec:design}).
  \item A measured comparison of throughput, p50/p95/p99 latency, and
        ...
        intervals on every percentile (Section~\ref{sec:results-steady}).
  ...
  \item An analytic cost model, calibrated against the measured
        ...
        cost-versus-latency choices (Section~\ref{sec:results-cost}).
\end{enumerate}
...
us-east-1 pricing puts the Lambda crossover at roughly \textbf{8\,RPS}
against EC2 and \textbf{10\,RPS} against Fargate.

The remainder of the paper is organised as follows.
Section~\ref{sec:background} situates the three paradigms and their billing.
Section~\ref{sec:design} describes the application and the unified deployment
pipeline. Section~\ref{sec:methodology} explains the workload, metrics, and
statistical procedure. Section~\ref{sec:results} reports steady-state,
