...
\setbeamertemplate{footline}{%
  \hfill\textcolor{black!45}{\scriptsize
    CS5296 Group~24 \quad
...
\begin{frame}{Four scenarios, driven by the same k6 harness}
  ...
  \begin{columns}[T]
    \begin{column}{0.46\linewidth}
      \begin{block}{Steady-state RPS sweep}
        Constant arrival rate at four levels (50/100/200/400 RPS for EC2 and Fargate; 25/50/75 for Lambda). Recovers per-stage tail latency.
      \end{block}
      \vspace{4pt}
      \begin{block}{Step-arrival burst}
        One minute idle, three minutes at the target rate, one minute idle. Captures elasticity and time-to-steady-state.
      \end{block}
    \end{column}
    \begin{column}{0.46\linewidth}
      \begin{block}{Cold-start probe}
        Twelve sparse \texttt{/healthz} requests with 60-second gaps, designed to characterise warm-vs-cold latency on Lambda.
      \end{block}
      \vspace{4pt}
      \begin{block}{Mixed read/write}
        70\,\% GET-redirect, 30\,\% POST-shorten at 200\,RPS for three minutes. Approximates real shortener traffic.
      \end{block}
    \end{column}
  \end{columns}
  ...
  \vspace{8pt}
  \begin{center}
    \footnotesize\textcolor{black!60}{All scenarios driven from a co-located \texttt{t3.small} k6 worker in \texttt{us-east-1a}; raw timings recorded in CSV; bootstrap 95\,\% CIs on every reported percentile.}
  \end{center}
\end{frame}
...
\begin{frame}{Cost: a single break-even crossover near 20 RPS}
  ...
  \vspace{6pt}
  \begin{itemize}\setlength\itemsep{2pt}
    \item Below \(\sim\)20\,RPS, Lambda's no-idle billing wins because EC2 and Fargate pay around the clock.
    \item Above \(\sim\)20\,RPS, a single provisioned instance amortises across enough requests that the flat hourly rate beats per-invocation pricing.
    \item Pareto chart at 50\,RPS: Lambda is strictly dominant in our measured regime.
  \end{itemize}
\end{frame}
